...
import { ADMIN_EMAIL, buildBatchBody, type DraftRow } from "../lib/overlay";
...
export function ReviewChangesPanel({
  ...
  const subject = `${TEAM.branding.appName} — ${draft.length} staged star${draft.length === 1 ? "" : "s"} to add`;
  const href = `mailto:${ADMIN_EMAIL}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
  // Long mailto: URLs get silently truncated by some clients — offer copy/paste instead.
          ...
          <Card.Body className="d-flex flex-column gap-2">
            {tooLong && (
              <Alert variant="info" className="mb-0 small">
                ...
                <strong>Copy rows</strong> and paste them into the email, or copy from the box
                below.
              </Alert>
            )}
            {submitted && (
              <Alert variant="success" className="mb-0 small">
                ...
                <strong>Remove All</strong> so they aren't counted twice on your device.
              </Alert>
            )}
              ...
              <Col sm={8} className="text-start">
                <details>
                  <summary className="small text-body-secondary" style={{ cursor: "pointer" }}>
                    Show rows to manually copy into email
                  </summary>
                  <Button
