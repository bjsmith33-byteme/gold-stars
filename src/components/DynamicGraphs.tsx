...
import { personChartSeries, type PersonStats } from "../lib/aggregate";
import { useBsTheme, type BsTheme } from "../lib/theme";
...
function axisColors(theme: BsTheme) {
  return theme === "dark"
...
export function DynamicGraphs({ stats }: { stats: PersonStats }) {
  ...
  return (
    <div className="mb-3">
      ...
      <div style={{ position: "relative", height: "12rem" }}>
        {/* Remount on any of these so Chart.js rebuilds cleanly rather than mutating. */}
        <Chart
          key={`${chartType}-${faceted}-${theme}`}
          type={chartType}
          data={data}
          options={options}
        />
